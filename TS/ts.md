- [TypeScript](#typescript)
  ...
  - [2. 进阶](#2-进阶)
    - [2.1 类型别名](#21-类型别名)
    - [2.2 字符串字面量类型](#22-字符串字面量类型)
    - [2.3 元祖（Tuple）](#23-元祖tuple)
    - [2.4枚举（enum）](#24枚举enum)
    - [2.5 类](#25-类)
    - [2.6 类与接口](#26-类与接口)
    - [2.7 泛型（Generics）](#27-泛型generics)
  - [2. 变量声明](#2-变量声明)
  - [3. 接口](#3-接口)
  - [问题](#问题)
## TypeScript
...
2. void：与 any 相反，表示没有任何类型。一个没有返回值的函数，其返回的类型是 void；而**一个 void 类型的变量，只能赋值 undefined 和 null**。
...
枚举`enum Color { Red， Green }`是对 JS 标准数据类型的一个补充，可以为一组**数值**赋予友好的名字：
...
正常情况先，比较两种不同的类型时，并不在乎他们从何而来，只要成员类型兼容，就可以认为二者兼容。然而，private 和 protectd 成员，需要来自同一处声明才会被认为是兼容的。
...
存在构造函数时，一般会在子类的构造函数中通过 super 调用超类的构造函数。
...
2.  Object：非原始类型，也就是除了 number, string, boolean, symbol, null 或 undefined 之外的类型。
...
在属性名不确定的时候，任意属性可以替代可选属性达到需要的效果
...
**函数类型**
使用接口表示函数类型，需要给接口定义一个调用签名；它就像一个只有参数列表和返回值类型的函数定义：
interface func {
   (source: string ,substring: string): boolean;
}

函数的参数名不需要与接口中定义的名字匹配；同时, 没有指定类型的函数，ts 的类型系统会自动推测并校验。

**可索引类型**
具有一个索引签名，描述了索引的类型，以及相应的索引返回值类型。 索引类型只能是 字符串和数字，但数字索引的返回值类型必须是字符串索引返回值类型的子类型；因为 js 会将 number 转换成 string 再去索引对象。

**类类型**
TS 用接口来明确的强制一个类去符合某种契约，它描述了类的公共部分(成员变量，方法)；它不会帮助检查类是否具有某些私有成员。

类有两个类型，静态和实例。类实现接口后，只会对实例部分进行类型检查；constructor 存在于类的静态部分，所以不在检查的范围内。
...
//可兼容 func(data: {type: string}): boolean {} 这样的函数

interface InterfaceB {
   doSomething: (data: object) => boolean //function as property declaration
}
//不可兼容 func(data: {type: string}): boolean {} 这样的函数，需要写具体才行
//属性可选或者可能是多个类型（'| string | number'）时更好写。
```
参考：[TypeScript Interface Function Property: What's the difference?](https://stackoverflow.com/questions/60653558/typescript-interface-function-property-whats-the-difference)
...
当接口继承一个类类型时，会继承所有成员(包括 private 和  protected)，但不包括实现。因此该接口只能被该类或该类的子类实现，在庞大的继承结构中有用。


参考：
- [TS 中文文档](https://www.tslang.cn/docs/handbook/interfaces.html)
